...
static int sTest = -1;
static int arc_tx_flag = 1;
static int arc_rx_flag = 0;
...
static void inv478x_event_callback_func(inv_inst_t inst, inv478x_event_flags_t event_flags)
{
    ...
    if (event_flags & INV478X_EVENT_FLAG__RX0_PLUS5V) {
        ...
        inv478x_rx_plus5v_query(inst, 0, &plus5v);
        if (!plus5v) {
            printf("\nbit 15 : EARC CEC TX STOP");
            if (arc_tx_flag) {
                inv_rx_cec_arc_tx_stop();
            }
            if (arc_rx_flag) {
                inv_tx_cec_arc_rx_stop();
            }
        }
        /*
         * bool_t val;
         * uint32_t ret_val;
         * printf("\nbit 2  : Rx Zero 5V Changed");
         * ret_val = inv478x_rx_plus5v_query(inst,0, &val);
         * if(!val){
         * enum inv_hdmi_earc_mode val;
         * uint32_t ret_val;
         * ret_val = inv478x_earc_mode_get(inst, &val);
         * if(val != INV_HDMI_EARC_MODE__EARC){
    ...
    //  ret_val = inv478x_rx_packet_query(inst, port, type, &p_packet);
    //       vic = p_packet.b[6];
    ...
    case 'h':
        {
            ...
            case 'a':
                {
                    printf("\n Enter HDCP2.3 License String : ");
                    fgets(val, 89, stdin);
                    printf("\n String Entered Is-\n%s", val);
                    ret_val = inv478x_license_cert_set(inst, val);
                    if (!ret_val)
                        printf("\n The HDCP2.3 License value has been set");
                    else
                        printf("\n Return value = %d", ret_val);
                    break;
                }
    ...
    case 'k':
        {
            ...
            ret_val = inv478x_license_id_query(inst, &val);
            if (!ret_val)
                printf("\n HDCP2.3 Cert ID is = 0x%lX\n", val);
            else
                printf("\n Return value = %d", ret_val);
            break;
        }
            ...
            case 'a':
                {
                    ...
                    printf("\n **Press 0 to EDID Replicate Disable");
                    printf("\n **Press 1 to EDID Replicate Enable ");
                    scanf("%d", &val);
                    ret_val = inv478x_rx_edid_replicate_set(inst, port, &val);
                    if (ret_val == 0)
                        printf("\n EDID Replicate Set Value on rx_port %d to tx_port %d is = %d", port, tx_port, val);
                    else
                        printf("\n API Error Code Return Value = %ld", ret_val);
                    break;
                }
            ...
            case 'a':
                {
                    ...
                    printf("\n **Press 1 to HPD Replicate Enable Level ");
                    printf("\n **Press 2 to HPD Replicate Enable EDGE ");
                    scanf("%d", &val);
    ...
    case 't':
        {
            ...
            scanf("%d", &type);
            ret_val = inv478x_rx_packet_query(inst, port, type, len, p_packet);
            if (!ret_val) {
...
void case_cec(void)
{
    ...
    case 'a':
        {
            inv_cec_tx_delete();
            inv_cec_rx_delete();
            inv_cec_tx_create(1, &inv9612_app_tx_notification, adpt_inst_9612_tx);
            arc_tx_flag = 0;
            arc_rx_flag = 1;
            printf("\nCEC Tx Create done");
            cec_created = 1;
    ...
    }                           /* switch (key_api_num) for CEC ends here */
}
