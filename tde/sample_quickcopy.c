...
hi_s32 main(void)
{
    ...
    ret = gfx2d_create_surface_by_file(ARGB8888_1024_683, &src_surface);
    if (ret != HI_SUCCESS) {
        return ret;
    }
    ...
    surface_list.ops_mode = HI_UNF_GFX2D_OPS_QUICK_COPY;

    ret = hi_unf_gfx2d_bitblit(handle, &surface_list);
    if (ret != HI_SUCCESS) {
        SAMPLE_GFX2D_PRINT("hi_unf_gfx2d_bitblit failed\n");
    }
