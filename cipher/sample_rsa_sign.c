/*
 * Copyright (C) hisilicon technologies co., ltd. 2019-2019. all rights reserved.
 * Description: drivers of sample_rsa_sign
 * Author: zhaoguihong
 * Create: 2019-06-18
 */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
#include <unistd.h>

#include "hi_type.h"
#include "hi_unf_system.h"
#include "hi_unf_cipher.h"

#define HI_ERR_CIPHER(format, arg...)  HI_PRINT("\033[0;1;31m" format "\033[0m", ##arg)
#define HI_INFO_CIPHER(format, arg...) HI_PRINT("\033[0;1;32m" format "\033[0m", ##arg)
#define TEST_END_PASS()         \
    HI_INFO_CIPHER("****************** %s test PASS !!! ******************\n", __FUNCTION__)
#define TEST_END_FAIL()         \
    HI_ERR_CIPHER("****************** %s test FAIL !!! ******************\n", __FUNCTION__)
#define TEST_RESULT_PRINT()            \
    {                        \
        if (ret)             \
            TEST_END_FAIL(); \
        else                 \
            TEST_END_PASS(); \
    }

static hi_void print_buffer(const char *string, const hi_u8 *input, hi_u32 length)
{
    hi_u32 i = 0;

    if (string != NULL) {
        printf("%s\n", string);
    }

    for (i = 0; i < length; i++) {
        if ((i % 16 == 0) && (i != 0)) {
            printf("\n");
        }
        printf("0x%02x ", input[i]);
    }
    printf("\n");

    return;
}

static unsigned char test_data[] = { "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" };
static unsigned char sha256_sum[32] = {
    0x24, 0x8D, 0x6A, 0x61, 0xD2, 0x06, 0x38, 0xB8, 0xE5, 0xC0, 0x26, 0x93, 0x0C, 0x3E, 0x60, 0x39,
    0xA3, 0x3C, 0xE4, 0x59, 0x64, 0xFF, 0x21, 0x67, 0xF6, 0xEC, 0xED, 0xD4, 0x19, 0xDB, 0x06, 0xC1,
};

static hi_u8 N[]  = {
    
};
hi_u8 E[]  = {
    0x01, 0x00, 0x01
};
hi_u8 D[]  = {
    ,
};
hi_u8 P[]  = {
    ,
};
hi_u8 Q[] = {
    ,
};
hi_u8 DP[] = {
    ,
};
hi_u8 DQ[] = {
    ,
};
hi_u8 QP[] = {
    ,
};

hi_u8 RES[] = {
    ,
};


hi_s32 rsa_sign_verify(hi_unf_cipher_rsa_sign_scheme en_scheme)
{
    hi_s32 ret;
    hi_u8 u8_sign[256];
    hi_u32 sign_len;
    hi_unf_cipher_rsa_sign_param rsa_sign;
    hi_unf_cipher_rsa_verify_param rsa_verify;
    hi_unf_cipher_rsa_sign_verify_data rsa_sign_data;
    hi_unf_cipher_rsa_sign_verify_data rsa_verify_data;
    ret = hi_unf_cipher_init();
    if (ret != HI_SUCCESS) {
        return HI_FAILURE;
    }

    memset(&rsa_sign, 0, sizeof(hi_unf_cipher_rsa_sign_param));
    rsa_sign.sign_scheme = en_scheme;
    rsa_sign.priv_key.n = N;
    rsa_sign.priv_key.d = D;
    rsa_sign.priv_key.n_len= sizeof(N);
    rsa_sign.priv_key.d_len = sizeof(D);

    rsa_sign_data.input = test_data;
    rsa_sign_data.input_len = sizeof(test_data) - 1;
    rsa_sign_data.hash_data = sha256_sum;
    rsa_sign_data.sign = u8_sign;
    rsa_sign_data.sign_len = &sign_len;
    ret = hi_unf_cipher_rsa_sign(&rsa_sign, &rsa_sign_data);
    if (ret != HI_SUCCESS) {
        HI_ERR_CIPHER("hi_unf_cipher_rsa_sign_param failed\n");
        return HI_FAILURE;
    }

    switch (en_scheme) {
        case HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256:
            if (memcmp(u8_sign, RES, sizeof(RES)) != 0) {
                HI_ERR_CIPHER("hi_unf_cipher_rsa_sign_param failed\n");
                print_buffer("sign", u8_sign, sign_len);
                print_buffer("golden", RES, sizeof(RES));
                return HI_FAILURE;
            }
            break;
        default:
            break;
    }

    memset(&rsa_verify, 0, sizeof(hi_unf_cipher_rsa_verify_param));
    rsa_verify.sign_scheme = en_scheme;
    rsa_verify.pub_key.n = N;
    rsa_verify.pub_key.e = E;
    rsa_verify.pub_key.n_len = sizeof(N);
    rsa_verify.pub_key.e_len = sizeof(E);

    rsa_verify_data.input = test_data;
    rsa_verify_data.input_len = sizeof(test_data) - 1;
    rsa_verify_data.hash_data = sha256_sum;
    rsa_verify_data.sign = u8_sign;
    rsa_verify_data.sign_len = &sign_len;

    ret = hi_unf_cipher_rsa_verify(&rsa_verify, &rsa_verify_data);
    if (ret != HI_SUCCESS) {
        HI_ERR_CIPHER("hi_unf_cipher_rsa_verify_param failed\n");
        return HI_FAILURE;
    }

    TEST_END_PASS();

    hi_unf_cipher_deinit();

    return HI_SUCCESS;
}

int main(int argc, char *argv[])
{
    hi_s32 ret;

    ret = rsa_sign_verify(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_V15_SHA256);
    if (ret != HI_SUCCESS) {
        HI_ERR_CIPHER("rsa_sign_verify failed\n");
        return HI_FAILURE;
    }
    ret = rsa_sign_verify(HI_UNF_CIPHER_RSA_SIGN_SCHEME_RSASSA_PKCS1_PSS_SHA256);
    if (ret != HI_SUCCESS) {
        HI_ERR_CIPHER("rsa_sign_verify failed\n");
        return HI_FAILURE;
    }

    return HI_SUCCESS;
}

