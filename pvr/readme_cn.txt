...
命令格式: sample_pvr_play file_name [vo_format]
      ...
      启动播放后如下接口菜单将会显示，可根据菜单提示完成相应的功能:
         n: 正常播放
         p: 暂停
         f: 快进(2x/4x/8x/12x/16x/24x/32x/64x)
         s: 慢放(2x/4x/8x/16x/32x/64x)
         r: 快退(1x/2x/4x/8x/12x/16x/24x/32x/64x)
         t: 逐帧播放
         k: 跳转到文件头
         e: 跳转到文件尾
         l: 向后跳转5秒
...
功能描述: sample_pvr_file_ctrl 用于展示PVR的回绕录制文件离线线性化和文件截断功能.
    示例程序支持对回绕的录制文件进行线性化；对录制文件进行截断，截取头部、尾部，并支持设定同步异步方式以及输出到新文件或在源文件上进行操作.


命令格式: sample_pvr_timeshift in_file_name ctrl_type sync_flag head_offset, end_offset same_file_flag
        in_file_name: 待处理文件
        ctrl_type: 功能选择；0 - 离线线性化; 1 - 文件截取
        sync_flag(file truncate): 同步异步标志；0 - 异步; 1 - 同步
        head_offset(file truncate): 截取头部长度
        end_offset(file truncate): 截取尾部长度
        same_file_flag(file truncate): 文件输出标志；0 - 输出到新文件;1 - 在源文件上修改
