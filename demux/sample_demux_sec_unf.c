...
hi_void ts_rx_thread(hi_void *args)
{
    ...
    while (!g_stop_ts_get_thread) {
        ...
        if (ret != HI_SUCCESS || acquired_num > 10) {
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
    ...
    play_attrs.chan_type = HI_UNF_DMX_CHAN_TYPE_SEC;
    play_attrs.buffer_size = 0x10000;
    ...
    ret = hi_unf_dmx_create_filter(PLAY_DMX_ID, &flt_attrs, &g_flt_handle);
    if (ret != HI_SUCCESS || g_flt_handle == HI_INVALID_HANDLE) {
        printf("call hi_unf_dmx_create_filter failed.\n");
        goto DMX_PLY_DESTROY;
    }

    ret = hi_unf_dmx_attach_filter(g_flt_handle, g_ply_handle);
    if (ret != HI_SUCCESS) {
        printf("call hi_unf_dmx_attach_filter failed.\n");
...
DMX_FLT_DETACH:
    hi_unf_dmx_detach_filter(g_flt_handle, g_ply_handle);

DMX_FLT_DESTROY:
    hi_unf_dmx_destroy_filter(g_flt_handle);
