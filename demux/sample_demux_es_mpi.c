...
hi_void ts_tx_thread(hi_void *args)
{
    ...
    while (!g_stop_ts_put_thread) {
        ...
        ret = hi_mpi_dmx_ram_put_buffer(g_ts_buf, readlen, 0);
        if (ret != HI_SUCCESS) {
...
hi_void ts_rx_thread(hi_void *args)
{
    ...
    hi_u32 acquired_num;
    dmx_buffer st_buf[10]; /* max es buffer is 10 */
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
    ...
    play_attrs.type = DMX_PLAY_TYPE_VID;
    ...
    while (1) {
        ...
        if ('r' == input_cmd[0]) {
            printf("change the pid!\n");
            ret = hi_mpi_dmx_play_close(g_ply_handle);
            if (ret != HI_SUCCESS) {
                printf("call hi_mpi_dmx_play_open failed.\n");
            }
            usleep(100 * 1000);
            printf("begin detach!\n");
            ret = hi_mpi_dmx_play_detach_pid_ch(g_ply_handle);
            if (ret != HI_SUCCESS) {
                printf("call hi_mpi_dmx_play_detach_pid_ch failed.\n");
            }
            printf("begin reattach!\n");
            ret = hi_mpi_dmx_play_attach_pid_ch(g_ply_handle, pid_ch_handle);
            if (ret != HI_SUCCESS) {
                printf("call hi_mpi_dmx_play_detach_pid_ch failed.\n");
            }
