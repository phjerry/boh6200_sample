...
hi_void ts_rx_thread(hi_void *args)
{
    ...
    hi_u32 acquired_num;
    dmx_buffer st_buf[10]; /* max ts buffer numbler is 10 */
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
    ...
    if (strcmp(argv[4], "pat") == 0) {
        table_id = PAT_TABLEID;
    } else if (strcmp(argv[4], "pmt") == 0) {
        table_id = PMT_TABLEID;
    } else {
    ...
    play_attrs.type = DMX_PLAY_TYPE_SEC;
    play_attrs.buf_size = 0x10000;
    ...
    flt_attrs.match[0] = table_id;

    ret = hi_mpi_dmx_play_create_filter(&flt_attrs, &g_flt_handle);
    if (ret != HI_SUCCESS || g_flt_handle == HI_INVALID_HANDLE) {
        printf("call hi_mpi_dmx_play_create_filter failed.\n");
        goto DMX_PLY_DETACH;
    }

    ret = hi_mpi_dmx_play_add_filter(g_ply_handle, g_flt_handle);
    if (ret != HI_SUCCESS || g_flt_handle == HI_INVALID_HANDLE) {
        printf("call HI_MPI_DMX_Ply_AddFilter failed.\n");
        goto DMX_FLT_DESTROY;
    }
...
DMX_FLT_DEL:
    hi_mpi_dmx_play_del_filter(g_ply_handle, g_flt_handle);

DMX_FLT_DESTROY:
    hi_mpi_dmx_play_destroy_filter(g_flt_handle);
