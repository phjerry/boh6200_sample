 ...
 * Description: keyladder asynchronous function sample.
...
/* SEPARATE_KEYSLOT means create keyslot and dsc separately */
//#define SEPARATE_KEYSLOT
...
hi_u8 g_session_key[KEY_LEN] = {
    0x66, 0x47, 0x9C, 0x36, 0x6F, 0x2C, 0x19, 0xC7,
    0x2D, 0x3A, 0x12, 0xB6, 0x75, 0x0F, 0x26, 0x98
};
...
hi_s32 set_key(hi_handle handle_klad)
{
    ...
    call_back.done_callback = klad_call_back;
    call_back.user_data = g_user_data;
    call_back.user_data_len = sizeof(g_user_data);
    ret = hi_unf_klad_async_set_content_key(handle_klad, &content_key, &call_back);
    if (ret != HI_SUCCESS) {
        klad_print_error_func(hi_unf_klad_async_set_content_key, ret);
        return ret;
    }
...
hi_s32 main(hi_s32 argc, hi_char *argv[])
{
...
#ifdef SEPARATE_KEYSLOT
    hi_unf_keyslot_attr ks_attr = {0};
#endif
    hi_unf_rootkey_attr rootkey_attr = {0};
    ...
    rootkey_attr.root_key_sel = HI_UNF_CAS_ROOTKEY_SLOT0;

    rootkey_attr.target_support.mcipher_support = HI_TRUE;
    rootkey_attr.target_support.tscipher_support = HI_TRUE;

    rootkey_attr.target_alg_support.aes_support = HI_TRUE;

    rootkey_attr.target_feature_support.decrypt_support = HI_TRUE;
    rootkey_attr.target_feature_support.encrypt_support = HI_TRUE;

    rootkey_attr.level = HI_UNF_ROOTKEY_LEVEL2;

    rootkey_attr.alg_support.aes_support = HI_TRUE;
    rootkey_attr.alg_support.sm4_support = HI_TRUE;
    rootkey_attr.alg_support.tdes_support = HI_TRUE;

    ret = hi_unf_klad_set_rootkey_attr(handle_klad, &rootkey_attr);
    if (ret != HI_SUCCESS) {
        klad_print_error_func(hi_unf_klad_set_rootkey_attr, ret);
        goto cipher_destroy;
    }
    ...
    attr_klad.klad_cfg.owner_id = 0x12345678;
    attr_klad.klad_cfg.klad_type = HI_UNF_KLAD_TYPE_DYNAMIC;
