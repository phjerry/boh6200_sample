...
int main(int argc, char *argv[])
{
    ...
    hi_tee_rootkey_attr rootkey_attr = {0};
    hi_tee_klad_session_key session_key = {0};
    ...
    rootkey_attr.rootkey_sel = HI_TEE_CAS_ROOTKEY_SLOT0;
    ...
    rootkey_attr.level = HI_TEE_ROOTKEY_LEVEL2;
    ...
    ret = tee_klad_set_rootkey_attr(handle_klad, &rootkey_attr);
    if (ret != 0) {
        klad_print_error_func(tee_klad_set_rootkey_attr, ret);
        goto out4;
    }

    attr_klad.klad_cfg.owner_id = 0x12345678;
    attr_klad.klad_cfg.klad_type = HI_TEE_KLAD_TYPE_DYNAMIC;
    ...
    session_key.level = HI_TEE_KLAD_LEVEL1;
    session_key.key_size = KEY_LEN;
    session_key.alg = HI_TEE_KLAD_ALG_TYPE_AES;
    memcpy(session_key.key, g_session_key, KEY_LEN);
    ret = tee_klad_set_session_key(handle_klad, &session_key);
    if (ret != 0) {
        klad_print_error_func(tee_klad_set_session_key, ret);
        goto out5;
    }
    ...
    ret = tee_klad_set_async_content_key(handle_klad, &content_key);
    if (ret != 0) {
        klad_print_error_func(tee_klad_set_async_content_key, ret);
        goto out5;
    }
