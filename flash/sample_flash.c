...
static char *device;
static hi_flash_type type = HI_UNF_FLASH_TYPE_MAX;
int type_set;
static char *file;
        ...
        switch (op) {
            ...
            case 'T':
                ...
                if (optarg[0] == 'N' || optarg[0] == 'n') {
                    type = HI_UNF_FLASH_TYPE_NAND_0;
                    printf("nand, ");
                } else if (optarg[0] == 'S' || optarg[0] == 's') {
                    type = HI_UNF_FLASH_TYPE_NOR_0;
                    printf("spi, ");
                } else if (optarg[0] == 'E' || optarg[0] == 'e') {
                    type = HI_UNF_FLASH_TYPE_EMMC_0;
                    printf("emmc, ");
                } else {
                    type_set = 0;
                    break;
            ...
            case 's':
                offset_set = 1;
                offset = strtoull(optarg, 0, BASE_HEX);
                printf("offset: 0x%llx, ", offset);
                break;
            case 'l':
                len_set = 1;
                len = strtoul(optarg, 0, BASE_HEX);
...
static int check_param(void)
{
    ...
    if (type >= HI_UNF_FLASH_TYPE_MAX || !type_set) {
    ...
    if (type != HI_UNF_FLASH_TYPE_EMMC_0 && (!device || !device[0])) {
        printf("this program require device param if flash is nand or spi!\n");
        return -1;
    }
    if (type == HI_UNF_FLASH_TYPE_EMMC_0 && !offset_set && (!device || !device[0])) {
        printf("emmc require device or offset at least!\n");
        return -1;
    }

    if (withoob && type != HI_UNF_FLASH_TYPE_NAND_0) {
...
{
    int i;
    for (i = 0; i < len / PRINT_STEP4; i += PRINT_STEP4) {
        if (i % PRINT_STEP16 == 0) {
            printf("%08x: ", i);
        }
        printf("%08x ", *(unsigned int*)(bf + i));

        if (((print_sn + 1) % PRINT_STEP4) == 0) {
            printf("\n");
        }
...
static int flash_sample_read(void)
{
    ...
    ret = hi_unf_flash_getinfo(handle, &info);
    if (ret) {
    ...
    if (info.page_size)
        cnt = step = info.page_size;
    else if (info.block_size) {
        if (BUF_SZ >= info.block_size)
            cnt = step = info.block_size;
        else
            cnt = step = BUF_SZ;
    } else {
    ...
    if (withoob) {
        ...
        cnt = info.page_size + info.oob_size;
    }

    if (type != HI_UNF_FLASH_TYPE_EMMC_0) {
        /* find max step */
        for (sz = 0; ((cnt + info.page_size) > BUF_SZ); sz++) {
            if (withoob) {
                step += info.page_size;
                cnt += info.page_size + info.oob_size;
            } else {
    ...
    for (; len >= BUF_SZ;) {
        ret = hi_unf_flash_read(handle, offset, tmp, cnt,
    ...
    if (len < BUF_SZ && len > 0) {
        ...
        if (withoob)
            len += (len / info.page_size) * info.oob_size;

        ret = hi_unf_flash_read(handle, offset, tmp, len,
                                withoob ? HI_FLASH_RW_FLAG_WITH_OOB : HI_FLASH_RW_FLAG_RAW);
        if (ret < 0 || ret != len) {
...
static int flash_sample_write(void)
{
    ...
    if (withoob) {
        cnt = info.oob_size + info.page_size;
        if (cnt > BUF_SZ) {
    ...
    if (info.page_size)
        printf("Write info: per pages:%d, write step(not include oob) :%d, " \
               "read count(may include oob, lie on -O) :%d\n", \
               step / info.page_size, step, cnt);
    else
        printf("Write info: Write step(not include oob) :%d, read count(may include oob, lie on -O) :%d\n",
               step, cnt);

    ret = hi_unf_flash_erase(handle, offset, len);
    if (type != HI_UNF_FLASH_TYPE_EMMC_0 && ret != len) {
    ...
    while (len >= step && !eof) {
        ...
        ret = hi_unf_flash_write(handle, offset, data, cnt,
                                 (withoob ? HI_FLASH_RW_FLAG_WITH_OOB : HI_FLASH_RW_FLAG_RAW));
        if (ret < 0 || cnt != ret) {
    ...
    if (len < BUF_SZ && len > 0 && !eof) {
        ...
        ret = hi_unf_flash_write(handle, offset, data, len,
                                 withoob ? HI_FLASH_RW_FLAG_WITH_OOB : HI_FLASH_RW_FLAG_RAW);
        if (ret < 0 || len != ret) {
            printf("Fail to write, ret:0x%x, offset:0x%llx, cnt:%d\n",
                   ret, offset, cnt);
...
int main(int argc, char *argv[])
{
    ...
    handle = hi_unf_flash_open(type, device);
