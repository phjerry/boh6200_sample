...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    if (HI_SUCCESS != Ret)
    {
        printf("call HI_UNF_PQ_Init failed 0x%x\n", Ret);
        goto VO_DEINIT;
    }
    ...
    {
        printf("call HI_UNF_PQ_SetDefaultParam failed 0x%x\n", Ret);
        goto PQ_DEINIT;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        printf("call HI_UNF_DISP_SetScreenOffset failed 0x%x\n", Ret);
        goto PQ_DEINIT;
    }
    ...
    Ret = HI_UNF_DMX_CreateTSBuffer(HI_UNF_DMX_PORT_RAM_0, &stTsBufAttr, &g_TsBuf);
    if (Ret != HI_SUCCESS)
    {
